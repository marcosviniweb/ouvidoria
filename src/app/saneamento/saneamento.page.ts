...
@Component({
  ...
  templateUrl: './saneamento.page.html',
  styleUrls: ['./saneamento.page.scss'],
})
export class SaneamentoPage implements OnInit {
photo: any;
  ...
  mostrarmapa() {
    ...
    const icon = {
      url: 'assets/imagens/icon/torneira.png',
  ...
  solicitacao(pedido) {
    ...
    if(this.pedido.descricao != '' && this.pedido.bairro != '' && this.pedido.endereco != '' && this.pedido.referencia){
      ...
      let img = `https://firebasestorage.googleapis.com/v0/b/app-prefeitura.appspot.com/o/Falta_de_saneamento%2F${this.protocolo}?alt=media`;
      this.pedido.situacao = "Aberto";
      // COLOCANDO O TIPO DE CATEGORIA DO PEDIDO
      this.pedido.categoria = "Falta de Saneamento";
  ...
  async tirarFoto() {
    ...
    let pictures = storage().ref(`Falta_de_saneamento/${this.protocolo}`);
