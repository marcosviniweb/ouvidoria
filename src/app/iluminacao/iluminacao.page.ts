...
@Component({
  selector: 'app-iluminacao',
  templateUrl: './iluminacao.page.html',
  styleUrls: ['./iluminacao.page.scss'],
})
export class IluminacaoPage implements OnInit {
  photo: any;
  pedido = {
  ...
  mostrarmapa() {
    ...
    const icon = {
      url: 'assets/imagens/icon/ilumina.png',
  ...
  solicitacao(pedido) {
    ...
    if(this.pedido.descricao != '' && this.pedido.bairro != '' && this.pedido.endereco != '' && this.pedido.referencia){
      ...
      let img = `https://firebasestorage.googleapis.com/v0/b/app-prefeitura.appspot.com/o/iluminacao%2F${this.protocolo}?alt=media`;
      this.pedido.situacao = "Aberto";
      // COLOCANDO O TIPO DE CATEGORIA DO PEDIDO
      this.pedido.categoria = "Ilumincação Pública";
  ...
  async tirarFoto() {
    ...
    let pictures = storage().ref(`iluminacao/${this.protocolo}`);
