...
@Component({
  selector: 'app-alagamento',
  templateUrl: './alagamento.page.html',
  styleUrls: ['./alagamento.page.scss'],
})
export class AlagamentoPage implements OnInit {
  photo: any;
  pedido = {
  ...
  mostrarmapa() {
    ...
    const icon = {
      url: 'assets/imagens/icon/alagamento.png',
  ...
  solicitacao(pedido) {
    ...
    if(this.pedido.descricao != '' && this.pedido.bairro != '' && this.pedido.endereco != '' && this.pedido.referencia){
      ...
      let img = `https://firebasestorage.googleapis.com/v0/b/app-prefeitura.appspot.com/o/alagamento%2F${this.protocolo}?alt=media`;
      this.pedido.situacao = "Aberto";
      // COLOCANDO O TIPO DE CATEGORIA DO PEDIDO
      this.pedido.categoria = "Problema de Alagamento";
  ...
  async tirarFoto() {
    ...
    let pictures = storage().ref(`alagamento/${this.protocolo}`);
