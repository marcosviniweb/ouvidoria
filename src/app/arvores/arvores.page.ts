...
export class ArvoresPage implements OnInit {
  ...
  mostrarmapa() {
    ...
    const icon = {
      url: 'assets/imagens/icon/arvores.png',
  ...
  solicitacao(pedido) {
    ...
    if(this.pedido.descricao != '' && this.pedido.bairro != '' && this.pedido.endereco != '' && this.pedido.referencia){
      ...
      let img = `https://firebasestorage.googleapis.com/v0/b/app-prefeitura.appspot.com/o/arvores%2F${this.protocolo}?alt=media`;
      this.pedido.situacao = "Aberto";
      // COLOCANDO O TIPO DE CATEGORIA DO PEDIDO
      this.pedido.categoria = "Árvores";
      this.pedido.tipo = "Solicitacao";
      let longitude = this.log;
      // INSERINDO NO BANCO DE DADOS  OS DADOS 
      this.pedido.valido = "Não";
      this.pedido.secretaria = "Secretaria de Meio Ambiente";
  ...
  async tirarFoto() {
    ...
    let pictures = storage().ref(`arvores/${this.protocolo}`);
