...
photo: any;
 ...
 // {title: 'Outro',id: '9'}
  ...
  mostrarmapa() {
    ...
    const icon = {
      url: 'assets/imagens/icon/emer.png',
  ...
  solicitacao(pedido) {
    ...
    if(this.pedido.descricao != '' && this.pedido.bairro != '' && this.pedido.endereco != '' && this.pedido.referencia){
      ...
      let img = `https://firebasestorage.googleapis.com/v0/b/app-prefeitura.appspot.com/o/emergencia%2F${this.protocolo}?alt=media`;
      this.pedido.situacao = "Aberto";
      // COLOCANDO O TIPO DE CATEGORIA DO PEDIDO
      this.pedido.categoria = "Emergência";
      ...
      this.afs.collection<Idea>("Solicitacoes/")
        .doc(dataid).set(
          {
            img: img, latitude: this.lat, secretaria: this.pedido.sec, mapa: mapa, longitude: longitude, id: this.pedido.id, tipo: this.pedido.tipo, protocolo: this.protocolo, situacao: this.pedido.situacao, data: dataid, dia: dia, categoria: this.pedido.categoria, bairro: this.pedido.bairro, descricao: this.pedido.descricao,
  ...
  async tirarFoto() {
    ...
    let pictures = storage().ref(`emergencia/${this.protocolo}`);
