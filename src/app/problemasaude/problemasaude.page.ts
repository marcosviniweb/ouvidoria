...
@Component({
  selector: 'app-problemasaude',
  templateUrl: './problemasaude.page.html',
  styleUrls: ['./problemasaude.page.scss'],
})
export class ProblemasaudePage implements OnInit {
  photo: any;
  pedido = {
  ...
  mostrarmapa() {
    ...
    const icon = {
      url: 'assets/imagens/icon/saude.png',
  ...
  solicitacao(pedido) {
    ...
    if(this.pedido.descricao != '' && this.pedido.bairro != '' && this.pedido.endereco != '' && this.pedido.referencia){
      ...
      let img = `https://firebasestorage.googleapis.com/v0/b/app-prefeitura.appspot.com/o/Problema_de_Saude%2F${this.protocolo}?alt=media`;
      this.pedido.situacao = "Aberto";
      // COLOCANDO O TIPO DE CATEGORIA DO PEDIDO
      this.pedido.categoria = "Problemas na Saúde";
      this.pedido.tipo = "Solicitacao";
      let longitude = this.log;
      // INSERINDO NO BANCO DE DADOS  OS DADOS 
      this.pedido.valido = "Não";
      this.pedido.secretaria = "Secretaria de Saúde";
  ...
  async tirarFoto() {
    ...
    let pictures = storage().ref(`Problema_de_Saude/${this.protocolo}`);
