...
declare var google;
@Component({
  selector: 'app-buracos',
  templateUrl: './buracos.page.html',
  styleUrls: ['./buracos.page.scss'],
})
export class BuracosPage implements OnInit {
  photo: any;
  pedido = {
  ...
  mostrarmapa() {
    ...
    const icon = {
      url: 'assets/imagens/icon/buracos.png',
  ...
  solicitacao(pedido) {
    ...
    if(this.pedido.descricao != '' && this.pedido.bairro != '' && this.pedido.endereco != '' && this.pedido.referencia){
      ...
      let img = `https://firebasestorage.googleapis.com/v0/b/app-prefeitura.appspot.com/o/Buracos%2F${this.protocolo}?alt=media`;
      this.pedido.situacao = "Aberto";
      // COLOCANDO O TIPO DE CATEGORIA DO PEDIDO
      this.pedido.categoria = "Buracos e Asfaltamentos";
  ...
  async tirarFoto() {
    ...
    let pictures = storage().ref(`Buracos/${this.protocolo}`);
