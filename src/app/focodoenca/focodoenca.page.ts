...
@Component({
  selector: 'app-focodoenca',
  templateUrl: './focodoenca.page.html',
  styleUrls: ['./focodoenca.page.scss'],
})
export class FocodoencaPage implements OnInit {
photo: any;
  pedido = {
  ...
  mostrarmapa() {
    ...
    const icon = {
      url: 'assets/imagens/icon/dengue.png',
  ...
  solicitacao(pedido) {
    ...
    if(this.pedido.descricao != '' && this.pedido.bairro != '' && this.pedido.endereco != '' && this.pedido.referencia){
      ...
      let img = `https://firebasestorage.googleapis.com/v0/b/app-prefeitura.appspot.com/o/Foco_Doenca%2F${this.protocolo}?alt=media`;
      this.pedido.situacao = "Aberto";
      // COLOCANDO O TIPO DE CATEGORIA DO PEDIDO
      this.pedido.categoria = "Foco de Doenças";
  ...
  async tirarFoto() {
    ...
    let pictures = storage().ref(`Foco_Doenca/${this.protocolo}`);
