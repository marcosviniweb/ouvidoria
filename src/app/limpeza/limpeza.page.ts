...
export class LimpezaPage implements OnInit {
  ...
  solicitacao(pedido) {
    ...
    if(this.pedido.descricao != '' && this.pedido.bairro != '' && this.pedido.endereco != '' && this.pedido.referencia){
      ...
      let img = `https://firebasestorage.googleapis.com/v0/b/app-prefeitura.appspot.com/o/limpeza%2F${this.protocolo}?alt=media`;
      this.pedido.situacao = "Aberto";
      // COLOCANDO O TIPO DE CATEGORIA DO PEDIDO
      this.pedido.categoria = "Limpeza e Coleta";
  ...
  async tirarFoto() {
    ...
    let pictures = storage().ref(`limpeza/${this.protocolo}`);
